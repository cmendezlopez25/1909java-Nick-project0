...
public class LotMenuImpl implements LotMenu {
	...
	public Menu display() {
		...
		sysout.println();
		sysout.println("Welcome back to the lot!");
	...
	private String validInput() {
		...
		if (SystemUtil.isInt(ret)) {
			if (lot.getCars() != null && lot.getCars().size() > 0
					&& (Integer.parseInt(ret) > lot.getCars().size() || Integer.parseInt(ret) < 0)) {
				ret = "-1";
	...
	private void addCar() {
		...
		Car car = new Car();
		sysout.println(
				"Enter the Vehicle Identification Number [Enter exactly 17 letters/numbers with no spaces or dashes]");
		car.setVin(enterVin());
		sysout.println("Enter the model");
		car.setModel(enterModel());
		sysout.println("Enter the year.");
		car.setYear(enterYear());
		sysout.println("Enter the base price.");
		car.setBasePrice(enterBasePrice());
	...
	private String enterVin() {
		...
		while (input == "-1") {
			...
			if (input.length() == 17) {
				...
				if (menuSystem.doesVinExist(input)) {
					input = "-1";
					sysout.println("VIN already exists. Try again.");
				}
			}
			else {
				input = "-1";
	...
	private String enterModel() {
		...
		String input = SystemUtil.nextLine();
		while (!SystemUtil.isValidInput(input, false, null)) {
			sysout.println("Invalid input. Try again.");
			input = SystemUtil.nextLine();
		}
	...
	private double enterBasePrice() {
		log.trace("Entering enterBasePrice");
		String input = SystemUtil.nextLine();
		while (!SystemUtil.isDouble(input) && Double.parseDouble(input) != 0.0) {
			sysout.println("Invalid input. Try again!");
			input = SystemUtil.nextLine();
		}
		log.trace("Exiting enterBasePrice");
		return Integer.parseInt(input);
	}
